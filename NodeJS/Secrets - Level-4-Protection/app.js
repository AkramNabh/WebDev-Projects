...
app.post("/register", async(req, res)=>{
    //level 4 is by salt hashing the password//
    //where it hashes it then adds specfic digits and hashes it//
    //this repeats by the number of saltRounds
    //and the return of it is a un-encryptable password.
    bcrypt.hash(req.body.password, saltRounds, function(err, hash) {
        const user = new User({
            email: req.body.username,
            // this turns the entered password to a hash password
            password: hash
        })
    });
    try{
