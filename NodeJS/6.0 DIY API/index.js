...
app.get("/jokes/:id", (req, res) =>{
  ...
  const resultJoke = jokes.find((joke) => joke.id === id);
...
app.post("/jokes",(req, res)=>{
  ...
  jokes.push(newJoke);
  //accessing the last index of an array / list
...
//  const id = parseInt(req.params.id);
//  jokes.find((joke) => joke.id === id).jokeText = req.body.text;
//  jokes.find((joke) => joke.id === id).jokeType = req.body.type;
//  const resultJoke = jokes.find((joke) => joke.id === id);
//  res.json(resultJoke);
...
app.patch("/jokes/:id",(req, res)=>{
  ...
  const patchedJoke = {
    id : id,
    //both lines in the bottom checks if the user sent an info to use in patching, else it passes the existing data
...
app.delete("/jokes/:id", (req, res)=>{
  ...
  if(searchIndex > -1){
    ...
    res.sendStatus(404).json({
      error: `joke with the id ${id} not found, no jokes were deleted`,
    })
  }
  console.log(`the joke with id ${req.params.id} has been deleted`);
})
//8. DELETE All jokes
app.delete("/all", (req, res)=>{
  ...
  if(userKey === masterKey){
    ...
    res.sendStatus(404).json({
      error: `you are not authorized to do such an action :D`,
    })
  }
})
app.listen(port, () => {
